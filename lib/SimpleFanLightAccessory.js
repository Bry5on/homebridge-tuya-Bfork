...
class SimpleFanLightAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        this.maxSpeed = parseInt(this.device.context.maxSpeed) || 100;
        this.fanDefaultSpeed = parseInt(this.device.context.fanDefaultSpeed) || 8;
    ...
    getSpeed(callback) {
        this.getState(this.dpRotationSpeed, (err, dp) => {
            if (err) return callback(err);
            const speed = this.convertRotationSpeedFromTuyaToHomeKit(this.device.state[this.dpRotationSpeed]);
            if (isNaN(speed) || !isFinite(speed)) {
                this.log.warn('Received invalid rotation speed value:', speed);
                return callback(new Error('Invalid rotation speed value'));
            }
            callback(null, speed);
        });
    }
    ...
    setSpeed(value, callback) {
        if (value === 0) {
            ...
            if (this.useStrings) {
                return this.setMultiStateLegacy({ [this.dpFanOn]: true, [this.dpRotationSpeed]: this.fanCurrentSpeed.toString() }, callback);
            } else {
                return this.setMultiStateLegacy({ [this.dpFanOn]: true, [this.dpRotationSpeed]: this.fanCurrentSpeed }, callback);
            }
        }
        callback();
    }
    ...
    getBrightness(callback) {
        const brightness = this.convertBrightnessFromTuyaToHomeKit(this.device.state[this.dpBrightness]);
        if (isNaN(brightness) || !isFinite(brightness)) {
            this.log.warn('Received invalid brightness value:', brightness);
            return callback(new Error('Invalid brightness value'));
        }
        callback(null, brightness / 10);
    }

    setBrightness(value, callback) {
        const brightness = this.convertBrightnessFromHomeKitToTuya(value)*10;
        if (isNaN(brightness) || !isFinite(brightness)) {
            this.log.warn('Received invalid HomeKit brightness value:', brightness);
            return callback(new Error('Invalid brightness value'));
        }
        return this.setState(this.dpBrightness, brightness, callback);
    }
    ...
    convertBrightnessFromTuyaToHomeKit(value) {
        ...
        return isNaN(brightness) || !isFinite(brightness) ? 0 : brightness;
    }

    convertBrightnessFromHomeKitToTuya(value) {
        const brightness = parseFloat(value) * 10;
        return isNaN(brightness) || !isFinite(brightness) ? 0 : brightness;
    }
}
